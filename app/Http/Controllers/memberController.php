    ...
    public function store(Request $request)
    {
        ...
        if ($existing) {
            ...
            DB::table('members')->where('user_id', $currentUser->id)->update([
                ...
                'membership_segment' => $request->membership,
                'rfid_no'            => $request->rfid,
                'last_active'        => $lastActiveValue, // 新增判斷更新
                'updated_at'         => now(),
            ]);
        } else {
            // 沒資料 → Insert
            $nextId = DB::table('members')->max('id') + 1;
            $memberCode = sprintf("U-%06d", $nextId);
        ...
        return redirect()->back()->with('success', '會員資料已成功儲存！');
    }
