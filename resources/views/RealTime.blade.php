<x-layouts.app :title="__('Real-Time Monitor')">
  <link rel="stylesheet" href="{{ asset('css/realtime.css') }}?v={{ time() }}">
  <div class="content">
    <!-- ====== Metrics Row ====== -->
    <div style="display: flex; gap: 24px; margin-bottom:24px;">
      ...
      <div style="flex: 1; background-color: #1e293b; border: 1px solid #3b82f6; border-radius: 8px; padding: 16px;">
        <div style="font-weight: bold;">In Use</div>
        <div>{{ $in_use ?? '0' }}</div>
      </div>
      <div style="flex: 1; background-color: #1e293b; border: 1px solid #3b82f6; border-radius: 8px; padding: 16px;">
        <div style="font-weight: bold;">Total Power (kW)</div>
        <div>{{ $disconnected_last_10m ?? '0' }}</div>
      </div>
      <div style="flex: 1; background-color: #1e293b; border: 1px solid #3b82f6; border-radius: 8px; padding: 16px;">
        <div style="font-weight: bold;">Average Utilization</div>
        <div>{{ isset($total_power_kw) ? number_format($total_power_kw, 1) : '0' }}</div>
      </div>
    </div>
    ...
    <div class="grid" >
      ...
      <!-- Station Map -->
      <section class="panel map map-fixed" >
        ...
        <div id="station-map-wrapper" style="position:relative;">
          <div id="station-map" style="height: 100px;"></div>
      ...
      <!-- Utilization Chart -->
      <section class="panel util util-fixed">
        <div class="cpt-chart-header">
          <h6 class="cpt-chart-title">Utilization (%)</h6>
          <div class="cpt-legend-line">
            <span class="cpt-line" style="background-color:#3b82f6;"></span>
            <span class="cpt-label">Utilization %</span>
          </div>
        </div>
        <canvas id="utilization-chart" style="width:100%; height:100px;"></canvas>
        <p class="cpt-chart-desc" style="color:white;">
          X: Minute/Second (Usage rate updated every second)<br>
          Y: % (Proportion of charging devices among all stations)
        </p>
      </section>

      <!-- Total Power Chart -->
      <section class="panel power power-fixed">
        <div class="cpt-chart-header">
          <h6 class="cpt-chart-title">Total Power (kWh)</h6>
          <div class="cpt-legend-line">
            <span class="cpt-line" style="background-color:#f59e0b;"></span>
            <span class="cpt-label">Total Power kWh</span>
          </div>
        </div>
        <canvas id="power-chart" style="width:100%; height:100px;"></canvas>
        <p class="cpt-chart-desc" style="color:white;">
          X: Minute/Second (Synchronized with the left chart to compare power changes)<br>
          Y: kWh (Represents total real-time output power of all chargers)
        </p>
      </section>
          ...
          <div class="query-main">
            <div class="query-box query-main-box">
              ...
              <div class="query-row">
                ...
                <div class="field">
                  <label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Temp.</label>
                  <select name="temp_operator" class="select" >
                    <option value=""></option>
                    <option value="gte">≥</option>
                    <option value="lte">≤</option>
                    <option value="eq">=</option>
                  </select>
                  <input type="text" name="temp_value" class="input" style="width:40px;">
              ...
              <div class="query-row">
                <div class="field">
                  ...
                  <input type="text" name="street" class="input" style="width:300px;">
                </div>
          ...
          <table class="device-table">
            <thead>
              <tr>
                ...
                <th onclick="sortTable('current')" data-field="current">Current (A) <span class="sort-icon" id="sort-current"></span></th>
                <th onclick="sortTable('voltage')" data-field="voltage">Voltage (V) <span class="sort-icon" id="sort-voltage"></span></th>
                <th onclick="sortTable('temp')" data-field="temp">Temperature (°C) <span class="sort-icon" id="sort-temp"></span></th>
                <th onclick="sortTable('time')" data-field="time">Update <span class="sort-icon" id="sort-time"></span></th>
                <th>Data Stream</th>
              </tr>
            </thead>
...
<script>
  ...
  if (mapEl) {
    ...
    stations.forEach(station => {
      ...
      const inuseCount = station.devices.filter(d => d.status === 'inuse').length;
      const totalCount = station.devices.length;

      let bgColor;
      if (availableCount > 0) {
        bgColor = '#16a34a'; // 綠色：有可用
      } else if (inuseCount === totalCount && totalCount > 0) {
        bgColor = '#2563eb'; // 藍色：全部 inuse
      } else {
        bgColor = '#ef4444'; // 紅色：全部沒有可用
      }
...
<script>
  ...
  new Chart(document.getElementById('utilization-chart'), {
    ...
    options: {
      ...
      scales: {
        y: { min: 0, max: 100, ticks: { stepSize: 5,color:'#ffffff' }, grid: { color: '#ccc' } },
        x: { ticks: { color:'#ffffff' },grid: { color: '#ccc' } }
      }
    }
  });
  ...
  new Chart(document.getElementById('power-chart'), {
    ...
    options: {
      ...
      scales: {
        y: { ticks: { color:'#ffffff' },grid: { color: '#ccc' } },
        x: { ticks: { color:'#ffffff' },grid: { color: '#ccc' } }
      }
    }
  });
